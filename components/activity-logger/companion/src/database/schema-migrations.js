...
class SchemaMigrations {
  ...
  async migrate(targetVersion = null) {
    ...
    console.log(`[MIGRATIONS] Running ${migrations.length} migration(s) from ${currentVersion} to ${target}`);
    ...
    for (const migration of migrations) {
      try {
        ...
        results.push({ migration: migration.name, version: migration.version, success: false, error: err.message });
  ...
  getMigrationsBetween(fromVersion, toVersion) {
    ...
    return migrations.filter(m => {
      if (isUpgrade) {
        return this.compareVersions(m.version, fromVersion) > 0 &&
    ...
    return [
          ...
          return new Promise((resolve, reject) => {
            ...
            `, (err) => {
              if (err) reject(err);
                ...
                db.db.run(`CREATE INDEX IF NOT EXISTS idx_conversations_workspace ON conversations(workspace_id)`, () => {});
                db.db.run(`CREATE INDEX IF NOT EXISTS idx_conversations_status ON conversations(status)`, () => {});
                resolve();
              }
            });
          });
      ...
      },
      {
        ...
        down: async (db) => {
          ...
          console.warn('[MIGRATION] Rollback for add_conversation_fields_to_prompts not fully supported');
        }
      },
      {
        version: '1.3.0',
          ...
          return new Promise((resolve, reject) => {
            ...
            `, (err) => {
              if (err) reject(err);
                ...
                db.db.run(`CREATE INDEX IF NOT EXISTS idx_audit_operation ON audit_log(operation_type)`, () => {});
                db.db.run(`CREATE INDEX IF NOT EXISTS idx_audit_workspace ON audit_log(workspace_id)`, () => {});
                db.db.run(`CREATE INDEX IF NOT EXISTS idx_audit_created ON audit_log(created_at)`, () => {});
                resolve();
              }
            });
          });
