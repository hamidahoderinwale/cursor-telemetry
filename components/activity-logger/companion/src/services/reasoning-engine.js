...
class ReasoningEngine {
  constructor() {
    ...
    this.hfApiKey =
      process.env.HF_TOKEN || process.env.HUGGINGFACE_API_KEY || process.env.HF_API_KEY || '';
  ...
  aggregateData(telemetryData, temporalContext) {
    ...
    const productivity = {
      avgEventsPerDay:
        stats.totalEvents / Math.max(1, Math.ceil((end - start) / (24 * 60 * 60 * 1000))),
      avgPromptsPerDay:
        stats.totalPrompts / Math.max(1, Math.ceil((end - start) / (24 * 60 * 60 * 1000))),
...
## AI Usage Patterns
${Object.entries(aiModes)
  .map(([mode, count]) => `- ${mode.toUpperCase()}: ${count} uses`)
  .join('\n')}
