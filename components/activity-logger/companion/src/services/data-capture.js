...
class DataCaptureService {
  constructor(rawData, persistentDB, cursorDbParser, ideStateCapture, promptCaptureSystem, statusMessageTracker) {
          ...
          if (dbFiles.length > 0) {
            ...
            try {
              ...
              if (this.rawData.cursorDatabase.conversations.length > 50) {
                this.rawData.cursorDatabase.conversations = this.rawData.cursorDatabase.conversations.slice(-50);
              }
  ...
  async syncPromptsFromCursorDB(db) {
    ...
    try {
      ...
      console.log(`[SYNC] Sync complete: ${newPrompts} new, ${skippedPrompts} skipped, ${this.syncedPromptIds.size} total tracked (${prompts.length} available in Cursor DB)`);
