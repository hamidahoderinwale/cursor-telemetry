/**
 * TF-IDF Computation Web Worker
 * Offloads expensive TF-IDF calculations to a worker thread
 */
...
function tokenizeCode(code) {
  ...
  // Extract identifiers
  const tokens = noComments.match(/[a-zA-Z_$][a-zA-Z0-9_$]*/g) || [];
 ...
 */
function computeTFIDF(files, options = {}) {
  const {
    maxFiles = 500,
  ...
  // Tokenize files
  const documents = filesToProcess.map(file => {
  ...
  const shouldLimit = totalPairs > maxSimilarities;

  if (shouldLimit) {
    // Use approximate method: only compute for files sharing common terms
    let computed = 0;
    
    for (let i = 0; i < tfidfVectors.length && computed < maxSimilarities; i++) {
      for (let j = i + 1; j < tfidfVectors.length && computed < maxSimilarities; j++) {
        ...
        if (hasCommonTerms || computed < 500) {
...
self.onmessage = function(e) {
  ...
  try {
    if (type === 'computeTFIDF') {
      const result = computeTFIDF(payload.files, payload.options || {});
