    ...
    <div class="modal-content export-modal-content" style="max-width: 600px;">
      ...
      <div class="modal-body" style="padding: var(--space-md);">
        ...
        <div style="display: grid; gap: var(--space-md);">
          ...
          <!-- Privacy Hierarchy - Ordered by privacy level (highest to lowest) -->
          <div>
            <label class="form-label" style="font-size: var(--text-sm); font-weight: 600; margin-bottom: var(--space-xs); display: block;">Privacy Hierarchy</label>
            <div style="display: flex; flex-wrap: wrap; gap: var(--space-sm);">
              <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: var(--text-sm);">
          ...
          <div>
            ...
            <div id="advancedOptionsSection" style="display: none; margin-top: var(--space-sm); padding-top: var(--space-sm); border-top: 1px solid var(--color-border);">
              ...
              <!-- PII Redaction Options (only shown if Rung 1 is selected) -->
              <div id="rung1PIIOptions" style="display: none; margin-top: var(--space-sm); padding-top: var(--space-sm); border-top: 1px solid var(--color-border);">
                <div style="font-size: var(--text-sm); font-weight: 600; margin-bottom: var(--space-xs);">
                  Rung 1: PII Redaction Options
                  <span style="font-weight: normal; color: var(--color-text-muted); font-size: 10px;">(Select which PII types to redact in token-level data)</span>
                </div>
                <div style="display: grid; gap: var(--space-xs); grid-template-columns: repeat(2, 1fr);">
                  <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: var(--text-sm);">
                    <input type="checkbox" id="rung1RedactEmails" checked>
                    <span>Emails</span>
                  </label>
                  <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: var(--text-sm);">
                    <input type="checkbox" id="rung1RedactNames" checked>
                    <span>Names</span>
                  </label>
                  <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: var(--text-sm);">
                    <input type="checkbox" id="rung1RedactUrls" checked>
                    <span>URLs</span>
                  </label>
                  <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: var(--text-sm);">
                    <input type="checkbox" id="rung1RedactIpAddresses" checked>
                  ...
                  <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: var(--text-sm);">
                    <input type="checkbox" id="rung1RedactAllStrings" checked>
                    <span>All Strings</span>
                  </label>
                  <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: var(--text-sm);">
                    <input type="checkbox" id="rung1RedactAllNumbers" checked>
                    <span>All Numbers</span>
                  </label>
                </div>
                <div style="font-size: var(--text-xs); color: var(--color-text-muted); margin-top: var(--space-xs);">
                  Note: "All Strings" and "All Numbers" override individual PII detection. Uncheck for selective redaction.
                </div>
                
                <!-- Semantic Expressiveness Fuzzing -->
                <div style="margin-top: var(--space-md); padding-top: var(--space-md); border-top: 1px solid var(--color-border);">
                  <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--space-xs);">
                    <input type="checkbox" id="rung1FuzzSemanticExpressiveness">
                    <span>Fuzz Semantic Expressiveness</span>
                  </label>
                  <div style="font-size: var(--text-xs); color: var(--color-text-muted); margin-left: 24px;">
                    When enabled, preserves data type distinctions (FUNC, CLASS, VAR) but anonymizes actual names. This removes semantic meaning while preserving type information and code structure.
                  </div>
                </div>
              </div>
            </div>
...
async function executeExportWithOptions() {
  ...
  const types = {
    ...
    moduleGraph: document.getElementById('exportTypeModuleGraph').checked,
    rung1: document.getElementById('exportTypeRung1').checked,
    rung2: document.getElementById('exportTypeRung2').checked,
    rung3: document.getElementById('exportTypeRung3').checked
  };
  
  // Get PII redaction options for Rung 1
  const rung1PIIOptions = {
    redactEmails: document.getElementById('rung1RedactEmails')?.checked !== false,
    redactNames: document.getElementById('rung1RedactNames')?.checked !== false,
  ...
  // Get semantic expressiveness fuzzing option for Rung 1
  const rung1FuzzSemanticExpressiveness = document.getElementById('rung1FuzzSemanticExpressiveness')?.checked === true;
  
  const options = {
    abstractionLevel: abstractionLevel,
    rung1PIIOptions: types.rung1 ? rung1PIIOptions : null,
    rung1FuzzSemanticExpressiveness: types.rung1 ? rung1FuzzSemanticExpressiveness : false,
