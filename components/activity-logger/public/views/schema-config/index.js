...
class SchemaConfigView {
  ...
  render() {
    ...
    if (!this.schema || (this.schema && (!this.schema.tables || this.schema.tables.length === 0))) {
      container.innerHTML = `
        <div class="schema-config-view">
          ...
          <div class="loading-state">
            ...
            <div style="font-size: var(--text-lg, 1rem); font-weight: 500; color: var(--color-text, #F8FAFC); margin-bottom: var(--space-xs, 0.25rem);">
              Loading schema configuration...
            </div>
            <div style="font-size: var(--text-sm, 0.8125rem); color: var(--color-text-muted, #CBD5E1); text-align: center; max-width: 500px;">
              ${this.connectionError ? 
                `Connection error: ${this.connectionError.message || 'Unable to connect to companion service'}` : 
                'Fetching database schema and field configurations'}
            </div>
            ${this.connectionError ? `
              <button class="btn btn-primary" onclick="schemaConfigView.init()" style="margin-top: var(--space-md, 0.75rem);">
                Retry Connection
              </button>
            ` : ''}
          </div>
    ...
    container.innerHTML = `
      <div class="schema-config-view">
        ...
        <div class="schema-content">
          ${viewMode === 'json' 
            ? (window.renderSchemaConfigJsonEditor ? 
                window.renderSchemaConfigJsonEditor(this.schema, this.customFields) : 
                '<p>Loading JSON editor...</p>')
            : activeTab === 'export-import' 
              ? (window.renderSchemaConfigExportImportTab ? 
                  window.renderSchemaConfigExportImportTab() : 
                  '<p>Loading export/import...</p>')
              : `
            <div class="schema-sidebar">
              ...
              <div class="table-list" id="table-list">
                ${window.renderSchemaConfigTableList ? 
                  window.renderSchemaConfigTableList(this.schema, this.selectedTable) : 
                  '<p>Loading tables...</p>'}
              </div>
            </div>
            ...
            <div class="schema-main">
              ${this.selectedTable 
                ? (window.renderSchemaConfigTableDetails ? 
                    window.renderSchemaConfigTableDetails(this.selectedTable, this.schema, this.customFields) : 
                    '<p>Loading table details...</p>')
                : (window.renderSchemaConfigWelcome ? 
                    window.renderSchemaConfigWelcome(this.schema, this.customFields, this.connectionError, this.apiBase) : 
                    '<p>Loading...</p>')}
            </div>
          `}
        </div>
  ...
  renderCustomFields(fields) {
    return window.renderSchemaConfigCustomFields ? 
      window.renderSchemaConfigCustomFields(fields) : 
      '<p class="empty-state">No field configurations. All fields will be exported by default.</p>';
  }
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  window.renderSchemaConfigView = function(container) {
    ...
    // Show loading state immediately while initializing
    container.innerHTML = `
      <div class="schema-config-view">
        <div class="schema-header">
          <div class="header-content">
    ...
    // Initialize and render (will update the container when done)
