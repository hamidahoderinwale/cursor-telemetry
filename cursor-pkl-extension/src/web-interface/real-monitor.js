...
const ASTIntentClassifier = require('../intent-classification/ast-intent-classifier');
const ClioIntentService = require('../intent-classification/clio-intent-service');
...
class RealMonitor {
  ...
  async startMonitoring() {
    ...
    // Start periodic updates for live durations
  ...
  startPeriodicUpdates() {
    // Update live durations every 5 seconds
    this.updateInterval = setInterval(() => {
      if (this.broadcastCallback) {
        ...
        activeSessions.forEach(session => {
          const duration = this.getLiveDuration(session.id);
        ...
        this.broadcastCallback('live-durations-update', liveDurations);
      }
    }, 5000);
  }
  ...
  async logNotebookChanges(sessionId, filePath, notebookData, conversationContext) {
    try {
      ...
      // Extract prompts from markdown cells
      const prompts = this.extractPromptsFromNotebook(notebookData, sessionId);
      if (prompts.length > 0) {
        console.log(`Found ${prompts.length} prompts in notebook:`, filePath);
        for (const prompt of prompts) {
          await this.savePromptToStorage(prompt);
          
          // Create event for event queue if available
          if (this.eventQueue) {
            const correlationId = `notebook-prompt-${Date.now()}`;
            ...
            await this.eventQueue.enqueue(promptEvent);
            console.log(`Prompt event enqueued: ${promptEvent.id}`);
          }
        }
      }
      ...
      if (this.broadcastCallback) {
        this.broadcastCallback('session-updated', session);
        this.broadcastCallback('sessions-update', this.getActiveSessions());
      }
  ...
  /**
   * Extract prompts from notebook markdown cells
   */
  extractPromptsFromNotebook(notebookData, sessionId) {
    const prompts = [];
    
    if (!notebookData.cells) return prompts;
    ...
    notebookData.cells.forEach((cell, cellIndex) => {
      if (cell.cell_type === 'markdown') {
        ...
        const promptPatterns = [
          /(?:prompt|question|request|ask|help|can you|please|i need|i want|create|build|make|generate|write|code|implement|fix|debug|explain|show|how to|what is|why|when|where)/i
        ];
        
        // Check if this looks like a prompt
        const isPrompt = promptPatterns.some(pattern => pattern.test(content)) && 
                        content.length > 10 && 
                        content.length < 1000; // Reasonable prompt length
        
        if (isPrompt) {
          const prompt = {
            id: 'prompt-' + Date.now() + '-' + cellIndex,
            sessionId: sessionId,
            timestamp: new Date().toISOString(),
            role: 'user',
          ...
          prompts.push(prompt);
          console.log(`Extracted prompt from cell ${cellIndex}:`, content.substring(0, 100) + '...');
        }
      }
    });
   ...
   */
  async savePromptToStorage(prompt) {
    try {
      if (this.dataStorage) {
        await this.dataStorage.saveConversation(prompt);
        console.log('Prompt saved to storage:', prompt.id);
      }
    } catch (error) {
